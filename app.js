/* ============================================================
   Empire National — Sales Tool (v3)
   Adds: team-wide monthly plan, team progress, personal
   contribution to plan. Plan is one number for the whole team.
   ============================================================ */
...
/* ---------- Data ---------- */
const USERS = [
  { id:"b1", name:"Maria Castillo", initials:"MC" },
  { id:"b2", name:"Diego Romero",   initials:"DR" },
  { id:"b3", name:"Hanna Wójcik",   initials:"HW" },
  { id:"b4", name:"Tyler Brooks",   initials:"TB" },
];
...
const SEED_DEALS = [
  ...
  D("b2","Maersk USA","won",             LANE("LRD","TX","SAT","TX"), 5400, 22),
...
/* ---------- Team plan widget (full, used on Dashboard) ---------- */
...
/* ---------- Personal contribution widget (Workspace) ---------- */
...
/* ---------- Sidebar nav ---------- */
const NAV = [
  { key:"workspace", label:"Workspace", icon:ICONS.workspace, sub:"Your day: follow-ups, deals, contribution to the team plan" },
  { key:"pipeline",  label:"Pipeline",  icon:ICONS.pipeline,  sub:"Deals from new lead to won — drag cards between stages" },
  { key:"dashboard", label:"Dashboard", icon:ICONS.dashboard, sub:"Team plan, margin and performance — filter by broker and customer" },
  ...
  return `
    ...
    <div style="margin-top:18px">
      ${cardBox("Contribution to team plan", `${state.teamPlan.month} · plan ${fmt(p.plan)}`, `
        ...
        ${brokerEntries.sort((a,b)=>b.v-a.v).map(b => {
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="plan-preview">
            ...
            <div class="pp-row"><span>Will become</span><b>${p.pct}% → recalculated on save</b></div>
            <div class="pp-row"><span>Brokers</span><b>${USERS.length}</b></div>
            <div class="pp-row"><span>Average share per broker</span><b>${Math.round(100/USERS.length)}% if equal</b></div>
          </div>
...
function openNewDealModal(){
  ...
  $("#nd-save").addEventListener("click", () => {
    ...
    state.deals.unshift({
      ...
      lane:LANE(oc,os,dc,ds), value:val, marginPct:mar,
    });
